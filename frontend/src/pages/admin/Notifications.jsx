...
export default function AdminNotifications() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await getMyNotifications({ limit: 80, unreadOnly });
  ...
  const emptyText = useMemo(
    () =>
      unreadOnly
        ...
        : "No notifications yet. Payment, message, and course system updates will appear here.",
    [unreadOnly]
  );
  ...
  if (loading) return <Loader label="Loading admin notifications..." />;

  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl border border-white/10 p-6">
        <p className="text-xs tracking-[0.25em] text-white/60">ADMIN • NOTIFICATIONS</p>
        <h1 className="mt-2 text-3xl md:text-4xl font-extrabold">
          Admin <span className="text-gold">Notifications</span>
        </h1>
        <p className="mt-2 text-white/65">
          New payments, student messages, course updates, and platform events.
        </p>
      </div>
